...
void drawLimbs(int userId, int limbID1, int limbID2) {
  ...
  float limb1X = map(convertedJoint1.x, 0, displayWidth, 0, width);
  float limb1Y = map(convertedJoint1.y, 0, displayHeight, 0, height);
  float limb2X = map(convertedJoint2.x, 0, displayWidth, 0, width);
  float limb2Y = map(convertedJoint2.y, 0, displayHeight, 0, height);
...
void drawJoint(int userId, int jointID) {
  ...
  PVector convertedJoint = getJoint(userId, jointID);
...
// * Translation of kinect proportion to fullscreen proportions
  float headx = map(convertedHead.x, 0, displayWidth, 0, width);
  float heady = map(convertedHead.y, 0, displayHeight, 0, height);
...
void drawSkeleton(int userId) {
  ...
  int userColor = int(random(userClr.length));
  stroke(userColor);
